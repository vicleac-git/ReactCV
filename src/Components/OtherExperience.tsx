...
import type { OtherExperienceItem } from '../types';

interface OtherExperienceProps {
  items: OtherExperienceItem[];
}

function OtherExperience({ items }: OtherExperienceProps) {
  const [isCollapsed, setIsCollapsed] = useState(false);

  return (
    <section id="other-experience" className="py-6 md:py-10 bg-slate-900 relative">
      <div className="max-w-6xl mx-auto px-6 lg:px-8 z-10">
        <div className="sticky top-0 z-20 bg-slate-900/95 backdrop-blur-sm py-4 mb-12 -mx-6 px-6 border-b border-violet-500/10 transition-all duration-300">
          ...
          >
            <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-cyan-400">Otra experiencia</h2>
